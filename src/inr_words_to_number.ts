export function inrWordsToNumber(currencyWordArray: string[]): string {
  ...
  for (let i = 0; i < split2DArray.length; i++) {
    ...
    const currNumber = calcAbsNumberValueForLessThenCroreValue(lessThenCrorePart);
...
function splitArrayBy(currencyWordArray: string[], delimiter: string): string[][] {
...
function calcAbsNumberValueForLessThenCroreValue(lessThenCrorePart: string[]): number {

  const magnitude: Record<string, number> = {
    'lakh': 100000, 'thousand': 1000, 'hundred': 100,
  }
  let result = 0;
